...
export async function POST(req: Request) {
  try {
    ...
    const { itemName, categoryName, organizationId } = await req.json()

    if (!itemName || !organizationId) {
      return NextResponse.json({ error: 'Item name and organization ID are required' }, { status: 400 })
    }
