...
export default async function SettingsPage({
  ...
  let allLocations: { id: string, name: string, slug: string, portal_display_name?: string | null, created_at: string }[] = []
  ...
  if (organization && isOwnerOrManager) {
    ...
    const { data: locsData } = await supabase
      .from('locations')
      .select('id, name, slug, portal_display_name, created_at')
      .eq('organization_id', organization.id)
      .order('created_at', { ascending: true })
    
    if (locsData) {
      allLocations = locsData as typeof allLocations
    }
  }
