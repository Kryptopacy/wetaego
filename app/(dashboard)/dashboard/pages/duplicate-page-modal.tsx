...
export function DuplicatePageModal({ sourcePageId, sourceTitle, sourceSlug }: {
  sourcePageId: string
  sourceTitle: string
  sourceSlug: string
}) {
  ...
  const handleDuplicate = async (e: React.FormEvent) => {
    ...
    if (res?.serverError || res?.validationErrors) {
      toast.error(res?.serverError || 'Failed to duplicate location')
    } else {
    ...
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          ...
          >
            <div>
              <h3 className="text-lg font-bold text-white">Duplicate Location</h3>
              <p className="text-xs text-zinc-400 mt-1">
                Create a clone of &quot;{sourceTitle}&quot; including all menus, items, and categories.
              </p>
            </div>

            <form onSubmit={handleDuplicate} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold uppercase text-zinc-400 mb-1">New Location Name</label>
                <input
                  type="text"
                  value={title}
              ...
              <div className="flex items-center justify-end gap-2 pt-2">
                ...
                >
                  {isLoading ? 'Duplicating...' : 'Duplicate Location'}
                </button>
              </div>
            </form>
          </div>
