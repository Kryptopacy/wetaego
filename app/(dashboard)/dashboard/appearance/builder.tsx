  ...
  return (
    <div className="flex flex-col md:flex-row h-[calc(100vh-64px)] -m-4 md:-m-8 overflow-hidden bg-black">
      ...
      <div className="hidden md:block w-96 border-r border-zinc-800 bg-zinc-950 overflow-y-auto p-6 scrollbar-hide">
        <h1 className="text-2xl font-semibold text-white mb-2">Storefront Builder</h1>
        <p className="text-sm text-zinc-400 mb-6">Real-time appearance customization.</p>
        ...
        <DesignTokensEditor 
          ...
          onTokensChange={(newTokens) => {
            if (scope === 'global') setTokens(newTokens)
              ...
              // We'd ideally mutate `pages` state here if we wanted immediate local reflection across switches
            }
          }}
          isLiveBuilder={true}
          pageId={scope === 'global' ? undefined : scope}
              ...
              <div className="flex-1 overflow-y-auto p-4 scrollbar-hide pb-24">
                ...
                <div className="mb-6 p-4 bg-zinc-900 rounded-xl border border-zinc-800">
                  ...
                  <select 
                    value={scope}
                    onChange={(e) => setScope(e.target.value)}
                    className="w-full bg-zinc-800 text-white border-zinc-700 rounded-lg p-2.5 text-sm focus:ring-emerald-500 focus:border-emerald-500"
                  >
                    <option value="global">Global (All Pages)</option>
                    {pages.map(p => (
                      <option key={p.id} value={p.id}>Page: {p.title}</option>
                  ...
                  {scope !== 'global' && (
                    <p className="text-xs text-zinc-500 mt-2">
                      Changes here only apply to {selectedPage?.title}, overriding Global settings.
                    </p>
                  )}
                </div>
