  ...
  const manifest = {
    ...
    tools: [
      ...
      // ── Staff & Operations MCP Suite (Server-Side Bearer Authenticated) ──
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            status: { type: "string", enum: ["pending", "accepted", "preparing", "ready", "served", "completed"] }
          },
          additionalProperties: false
        }
      },
      ...
      },
      {
        ...
        description: "Change the operational status of an existing order (e.g. accepted -> preparing -> ready -> served).",
        inputSchema: {
          type: "object",
          required: ["orderId", "status"],
          properties: {
            orderId: { type: "string", description: "Target order ID." },
            status: { type: "string", enum: ["accepted", "preparing", "ready", "served", "completed", "cancelled"] },
