...
if (!typesContent.includes('loyalty_settings')) {
  // Use Exact matching ensuring it's not graphql_public
  typesContent = typesContent.replace(/^  public: \{\r?\n    Tables: \{/m, (match) => {
    return match + newTables;
  });
  
  fs.writeFileSync('lib/supabase/types.ts', typesContent)
  console.log('Successfully injected CRM tables exactly into public')
} else {
