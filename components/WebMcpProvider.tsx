...
export const WEBMCP_TOOLS: WebMCPTool<any, any>[] = [
  ...
  // 2. wetaego_search_catalog — page: '/'
  {
    ...
    page: '/',
    description:
      'Search products, dishes, services, and multi-industry inventory across all WETAEGO business verticals (dining, retail, boutique, electronics, wellness, spas, hospitality, stays, creator portfolios, and services). Supports size, color, condition, brand, duration, capacity, room type, and dietary filtering. Each returned item contains a unique "itemId" required for calling wetaego_get_item_details, wetaego_add_to_cart, and wetaego_recommend_pairings.',
    inputSchema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'Keyword search query for dishes, merchandise, services, or bookings.' },
        category: { type: 'string', description: 'Category name filter (e.g. "Main Courses", "Dresses", "Laptops", "Facials", "Suites").' },
        size: { type: 'string', description: 'Size filter for apparel, shoes, portions, or dimensions (e.g. "S", "M", "L", "XL", "42", "14-inch").' },
        color: { type: 'string', description: 'Color filter for retail & boutique merchandise (e.g. "black", "emerald", "navy", "white").' },
        condition: {
          type: 'string',
          enum: ['new', 'refurbished', 'pre_owned'],
          description: 'Product condition filter for retail, electronics, and devices.',
        },
        brand: { type: 'string', description: 'Brand or manufacturer filter (e.g. "Apple", "Nike", "Sony").' },
        dietary: {
          type: 'array',
          items: { type: 'string', enum: ['vegan', 'vegetarian', 'halal', 'kosher', 'gluten_free', 'dairy_free', 'nut_free', 'keto'] },
          description: 'Dietary requirement filter tags for dining and food venues.',
        },
        durationMinutes: { type: 'integer', minimum: 5, maximum: 480, description: 'Service duration filter in minutes for spas, salons, wellness, and consulting.' },
        guestCapacity: { type: 'integer', minimum: 1, maximum: 500, description: 'Minimum guest or seated capacity for hotel rooms, shortlets, and event venues.' },
        roomType: { type: 'string', description: 'Room or space type filter for hospitality (e.g. "single", "double", "suite", "penthouse", "villa", "banquet_hall").' },
        maxPrice: { type: 'number', minimum: 0, description: 'Maximum price filter in major currency units.' },
        inStockOnly: { type: 'boolean', default: true, description: 'Return only in-stock/available items or open booking slots.' },
        limit: { type: 'integer', minimum: 1, maximum: 100, default: 20, description: 'Max items per page (1-100).' },
        offset: { type: 'integer', minimum: 0, default: 0, description: 'Offset for skipping items (pagination).' },
      },
      additionalProperties: false,
    },
    outputSchema: {
