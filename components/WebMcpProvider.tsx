...
export const WEBMCP_TOOLS: WebMCPTool<any, any>[] = [
  ...
  // 2. wetaego_search_catalog — page: '/'
  {
    ...
    page: '/',
    description:
      'Search products, dishes, services, and multi-industry inventory (dining, retail, boutique, electronics, wellness, hospitality) on WETAEGO. Supports size, color, condition, brand, and dietary filtering. Each returned item contains a unique "itemId" required for calling wetaego_get_item_details, wetaego_add_to_cart, and wetaego_recommend_pairings.',
    inputSchema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'Keyword search query for dishes, items, or services.' },
        category: { type: 'string', description: 'Category name filter (e.g. "Main Courses", "Dresses", "Laptops", "Spa Treatments").' },
        size: { type: 'string', description: 'Size filter for apparel, shoes, or portions (e.g. "S", "M", "L", "XL", "42", "14-inch").' },
        ...
        inStockOnly: { type: 'boolean', default: true, description: 'Return only in-stock/available items.' },
