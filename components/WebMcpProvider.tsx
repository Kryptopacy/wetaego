...
export const WEBMCP_TOOLS: WebMCPTool<any, any>[] = [
  ...
  // 2. wetaego_search_catalog — page: '/'
  {
    ...
    page: '/',
    description:
      'Search products, dishes, services, and inventory on WETAEGO. Each returned item contains a unique "itemId" required for calling wetaego_get_item_details, wetaego_add_to_cart, and wetaego_recommend_pairings.',
    inputSchema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'Keyword search query for dishes, items, or services.' },
        category: { type: 'string', description: 'Category name filter (e.g. "Main Courses", "Spa Treatments", "Electronics").' },
        dietary: {
          type: 'array',
